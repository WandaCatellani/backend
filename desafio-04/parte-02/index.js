...
console.log(
  `Inicio. Una vez que ingreses texto, tienes ${
    time / 1000
  } segundos para escribir y ver las palabras en espejo. Apurate!`
);
  ...
  .subscribe(
    () => {
      if (textInput.value === "error") {
        ...
        console.error(
          "Como ha escrito la palabra 'error', se ha finalizado la función en modo error"
        );
        clean();
      }
      ...
      if (textInput.value === "complete") {
        suscriber.complete();
        console.warn(
          "Como ha escrito la palabra 'complete', se ha finalizado la función de forma completa"
        );
        clean();
      }
